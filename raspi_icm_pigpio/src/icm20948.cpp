...
const icm20948::CommunicationState& icm20948::initialize()
{
    ...
    gpioSetMode(PIGPIO_ICM_INTERRUPT_PIN, PI_INPUT);
    // Pull-up the pin
    gpioSetPullUpDown(PIGPIO_ICM_INTERRUPT_PIN, PI_PUD_UP);
    int interruptResult =
        gpioSetAlertFuncEx(PIGPIO_ICM_INTERRUPT_PIN, icm20948::handleInterrupt, this);
    ...
    mI2cDeviceFileDescriptor = i2cOpen(I2C_BUS_NUMBER, ICM20948REG::I2C_ICM_ADDRESS, 0);
    ...
    else
    {
        ...
        i2cWriteByteData(mI2cDeviceFileDescriptor, ICM20948REG::I2C_ICM_REG_BANK_SEL,
                         ICM20948REG::REG_BANK_SEL::SELECT_USER_BANK_0);
        // Reset the device
        i2cWriteByteData(mI2cDeviceFileDescriptor, ICM20948REG::I2C_ICM_PWR_MGMT_1,
        ...
        int whoAmIReturn = i2cReadByteData(mI2cDeviceFileDescriptor, ICM20948REG::I2C_ICM_WHO_AM_I);
        ...
        if(whoAmIReturn == ICM20948REG::I2C_ICM_WHO_AM_I_ANSWER)
        {
            ...
            int userbankSelected =
                i2cReadByteData(mI2cDeviceFileDescriptor, ICM20948REG::I2C_ICM_REG_BANK_SEL);
            ROS_INFO_STREAM(
                "[icm20948::initialize] The current userbank selected is : " << userbankSelected);

            // GYRO_SMPLRT_DIV
            i2cWriteByteData(mI2cDeviceFileDescriptor, ICM20948REG::I2C_ICM_GYRO_SMPLRT_DIV,
                             ICM20948REG::GYRO_SMPLRT_DIV::GYRO_SMPLRT_DIV_225HZ);
            // GYRO_CONFIG_1
            i2cWriteByteData(mI2cDeviceFileDescriptor, ICM20948REG::I2C_ICM_GYRO_CONFIG_1,
            ...
            // ACCEL_SMPLRT_DIV_2
            i2cWriteByteData(mI2cDeviceFileDescriptor, ICM20948REG::I2C_ICM_ACCEL_SMPLRT_DIV_2,
                             ICM20948REG::ACCEL_SMPLRT_DIV2::ACCEL_SMPLRT_DIV2_225HZ);
            // ACCEL_CONFIG
            i2cWriteByteData(mI2cDeviceFileDescriptor, ICM20948REG::I2C_ICM_ACCEL_CONFIG,
            ...
            i2cWriteByteData(mI2cDeviceFileDescriptor, ICM20948REG::I2C_ICM_INT_ENABLE_1,
                             ICM20948REG::INT_ENABLE_1::RAW_DATA_0_RDY_EN);
            // Interrupt pin
            int interruptEnable1Reg =
                i2cReadByteData(mI2cDeviceFileDescriptor, ICM20948REG::I2C_ICM_INT_ENABLE_1);
 ...
 * this class.
 */
void icm20948::handleInterrupt(int gpio, int level, uint32_t tick, void* imuPointer)
{
...
void icm20948::acquireImuReadings()
{
    ...
    // ACCEL_X
    i2cReadI2CBlockData(mI2cDeviceFileDescriptor, ICM20948REG::I2C_ICM_ACCEL_XOUT_H,
                        accelByteBuffer, 6);
    ...
    i2cReadI2CBlockData(mI2cDeviceFileDescriptor, ICM20948REG::I2C_ICM_GYRO_XOUT_H, gyroByteBuffer,
                        6);
    // GYRO_X
...
{
    i2cClose(mI2cDeviceFileDescriptor);
    gpioTerminate();
}
